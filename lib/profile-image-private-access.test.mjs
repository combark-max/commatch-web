...
const batchAccessMigration = readOptionalMigration(
  '../supabase/profile-images-batch-access.sql',
);
...
test('batch profile image requests authorize once through the batch RPC and authenticate once in their route', () => {
  ...
  assert.match(batchRoute, /'can_access_profile_images'/);
  assert.equal(batchRoute.match(/supabase\.rpc\(/g)?.length, 1);
  assert.doesNotMatch(batchRoute, /'can_access_profile_image'/);
...
test('batch profile image responses expose numeric stage durations without sensitive timing metadata', () => {
  assert.match(batchRoute, /const routeStartedAt = performance\.now\(\)/);
  assert.match(batchRoute, /authStartedAt = performance\.now\(\)/);
  assert.match(batchRoute, /accessStartedAt = performance\.now\(\)/);
  assert.match(batchRoute, /signStartedAt = performance\.now\(\)/);

  const timingStart = batchRoute.indexOf('const serverTiming = [');
  const timingEnd = batchRoute.indexOf("].join(', ');", timingStart);
  assert.ok(timingStart >= 0 && timingEnd > timingStart);

  const timingBlock = batchRoute.slice(timingStart, timingEnd);
  for (const metric of ['auth', 'access', 'sign', 'total']) {
    assert.match(timingBlock, new RegExp(`${metric};dur=\\$\\{formatDuration\\([^)]+\\)\\}`));
  }
  assert.doesNotMatch(
    timingBlock,
    /user|path|url|cookie|token|rpc|argument/i,
  );
  assert.match(batchRoute, /const formatDuration = \(duration: number\) => duration\.toFixed\(1\)/);
  assert.match(batchRoute, /response\.headers\.set\('Server-Timing', serverTiming\)/);
});

test('additive batch authorization migration reuses the scalar policy with bounded invoker-only access', () => {
  assert.match(
    batchAccessMigration,
    /function public\.can_access_profile_images\(p_object_paths text\[\]\)/,
  );
  assert.match(batchAccessMigration, /returns text\[\]/);
  assert.match(batchAccessMigration, /security invoker/);
  assert.match(batchAccessMigration, /set search_path = ''/);
  assert.match(batchAccessMigration, /public\.can_access_profile_image\(candidate\.object_path\)/);
  assert.match(batchAccessMigration, /pg_catalog\.cardinality\(p_object_paths\) > 50/);
  assert.match(batchAccessMigration, /with ordinality/);
  assert.doesNotMatch(
    batchAccessMigration,
    /create or replace function public\.can_access_profile_image\(p_object_path text\)/,
  );
  assert.doesNotMatch(batchAccessMigration, /security definer/i);
  assert.doesNotMatch(batchAccessMigration, /\b(?:create|alter|drop)\s+policy\b/i);
  assert.doesNotMatch(batchAccessMigration, /\b(?:update|delete from)\s+storage\./i);
  assert.doesNotMatch(
    batchAccessMigration,
    /(?:grant|revoke)[^;]*public\.can_access_profile_image\(text\)/i,
  );
  assert.doesNotMatch(batchAccessMigration, /\bexecute\b[^;]*p_object_paths/i);
  assert.match(
    batchAccessMigration,
  ...
  assert.match(
    batchAccessMigration,
    /revoke all on function public\.can_access_profile_images\(text\[\]\)[\s\S]*from public, anon, authenticated, service_role/,
  );
  assert.match(
    batchAccessMigration,
    /grant execute on function public\.can_access_profile_images\(text\[\]\)[\s\S]*to authenticated, service_role/,
  );
});
