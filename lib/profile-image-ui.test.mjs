...
test('profile image wrappers suppress context menus without global interception', () => {
  const wrapperImageFiles = [
    ...
    '../components/admin/AdminMemberProfileImages.tsx',
  ];

  for (const relativePath of wrapperImageFiles) {
    const source = readFileSync(new URL(relativePath, import.meta.url), 'utf8');
    assert.match(
      source,
      /onContextMenu=\{preventProfileImageContextMenu\}/,
      `${relativePath} must suppress context menus on a profile image wrapper`,
    );
  }

  const projectSources = wrapperImageFiles
    .map((relativePath) => readFileSync(new URL(relativePath, import.meta.url), 'utf8'))
    .join('\n');
  assert.doesNotMatch(projectSources, /document\.addEventListener\(['"]contextmenu['"]/);
  assert.doesNotMatch(projectSources, /onTouchStart=\{[^}]*preventDefault/);
});
