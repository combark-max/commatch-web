  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/account/find-email', {
        ...
        body: JSON.stringify({ nickname, birthDate, phone }),
      });
      const body = await response.json() as { found?: boolean; maskedEmail?: string; message?: string };
      if (!response.ok || !body.found || !body.maskedEmail) {
        setError(body.message ?? '입력한 정보와 일치하는 계정을 확인할 수 없습니다.');
      } else {
        setResult(body.maskedEmail);
      }
    } catch {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-gray-100 bg-white p-8 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="mt-7 space-y-5">
          ...
          <div>
            <label htmlFor="phone" className="mb-1 block text-sm font-medium text-gray-700">인증한 휴대폰 번호</label>
            <input id="phone" type="tel" autoComplete="tel" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="010-1234-5678" required className="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-100" />
          </div>
        ...
        <p className="mt-4 text-xs leading-relaxed text-gray-400">닉네임, 생년월일, 인증한 휴대폰 번호가 모두 일치하면 이메일 일부만 표시됩니다.</p>
