...
export type PushDrainResult =
  | { body: PushDrainCounts; status: 200 }
  | { body: { message: string }; status: 500 | 503 };
...
function resolveTargetId(
  ...
  if (!notification || notification.type !== claim.eventType) {
    return undefined;
  }
...
export async function runPushDrain(): Promise<PushDrainResult> {
  try {
    assertPushServerConfigured();
  } catch {
    console.error('Push worker configuration is missing or invalid.', { code: 'vapid_configuration_invalid' });
    return { body: { message: 'Push worker is unavailable.' }, status: 503 };
  }
  ...
  if (expansionError) {
    ...
    return { body: { message: 'Push drain failed.' }, status: 500 };
  }
